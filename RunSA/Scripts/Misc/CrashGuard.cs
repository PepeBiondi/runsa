...
namespace Server.Misc
{
	public class CrashGuard
	{
		...
		private static void SendEmail( string filePath )
		{
			...
			MailMessage message = new MailMessage( "GemUO", Email.CrashAddresses );

			message.Subject = "Automated GemUO Crash Report";

			message.Body = "Automated GemUO Crash Report. See attachment for details.";
		...
		private static void GenerateCrashReport( CrashedEventArgs e )
		{
			...
			try
			{
				...
				using ( StreamWriter op = new StreamWriter( filePath ) )
				{
					...
					op.WriteLine( "===================" );
					op.WriteLine();
					op.WriteLine( "GemUO Version {0}.{1}, Build {2}.{3}", ver.Major, ver.Minor, ver.Build, ver.Revision );
