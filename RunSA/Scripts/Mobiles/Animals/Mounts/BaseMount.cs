...
namespace Server.Mobiles
{
	public abstract class BaseMount : BaseCreature, IMount
	{
		...
		// Private notes
		// 1113730 You may not initiate combat while flying over such precarious terrain.
		// 1113749 You may not use that while flying over such precarious terrain.
		...
		public static bool OnFlightPath( Mobile m )
		{
			...
			Tile[] tiles = m.Map.Tiles.GetStaticTiles( m.X, m.Y, true );
			ItemData itemData;
			bool onpath = false;

			for ( int i = 0; i < tiles.Length && !onpath; ++i )
			{
				itemData = TileData.ItemTable[(tiles[i].ID-0x4000) & 0x7FFF];
