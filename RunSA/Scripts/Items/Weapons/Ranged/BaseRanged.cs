...
namespace Server.Items
{
	public abstract class BaseRanged : BaseMeleeWeapon
	{
		...
		{
			#region SA
			if ( attacker.Player && !defender.Player && (defender.Body.IsAnimal || defender.Body.IsMonster) && 0.4 >= Utility.RandomDouble() && !(this is BaseThrown) )
		...
		{
			#region SA
			if ( attacker.Player && 0.4 >= Utility.RandomDouble()  && !(this is BaseThrown) )
