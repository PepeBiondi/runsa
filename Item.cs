...
namespace Server
{
	...
	public class Item : IEntity, IHued, IComparable<Item>, ISerializable
	{
		...
		{
            if (dropped.Stackable && Stackable && dropped.GetType() == GetType() && dropped.ItemID == ItemID && dropped.Hue == Hue && dropped.Name == Name && (dropped.Amount + Amount) <= 60000 && InstanceOwner == dropped.InstanceOwner)
			{
		...
		public Rectangle2D GetGraphicBounds()
		{
			...
			#region SA
			Rectangle2D bounds = ItemBounds.Table[itemID & 0x7FFF];
			#endregion
        ...
        private object m_InstanceOwner;

        public object InstanceOwner
        {
            get { return m_InstanceOwner; }
            set { m_InstanceOwner = value; }
        }
		...
		public virtual bool DropToWorld( Mobile from, Point3D p )
		{
			...
			for ( int i = 0; i < tiles.Length; ++i )
			{
				Tile tile = tiles[i];
				#region SA
			// 	Fix for Z of item dropped onto statics
			//	ItemData id = TileData.ItemTable[tile.ID & 0x7FFF];
		...
		public ItemData ItemData
		{
			...
			{
				#region SA
				return TileData.ItemTable[m_ItemID & 0x7FFF];
				#endregion
			}
		}
