	...
	public class HouseFoundation : BaseHouse
	{
		...
		public void AddFixtures( Mobile from, MultiTileEntry[] list )
		{
			...
			for( int i = 0; i < list.Length; ++i )
			{
				...
				#region SA
				int itemID = mte.m_ItemID & 0x7FFF;
				#endregion
		...
		public static bool ValidPiece( int itemID, bool roof )
		{
			...
			if ( !roof && ( TileData.ItemTable[itemID & 0x7FFF].Flags & TileFlag.Roof ) != 0 )
				return false;
			else if ( roof && ( TileData.ItemTable[itemID & 0x7FFF].Flags & TileFlag.Roof ) == 0 )
				return false;
			#endregion
			...
			for( int i = 0; i < 4; ++i )
			{
				...
				if( ax >= 1 && ax < mcl.Width && ay >= 1 && ay < mcl.Height - 1 )
				{
					...
					for( int j = 0; !hasBaseFloor && j < tiles.Length; ++j )
						hasBaseFloor = (tiles[j].Z == 7 && (tiles[j].ID & 0x7FFF) != 1);
					#endregion
			...
			if( context != null )
			{
				...
				if( ax >= 1 && ax < mcl.Width && ay >= 1 && ay < mcl.Height - 1 )
				{
					...
					for( int i = 0; !hasBaseFloor && i < tiles.Length; ++i )
						hasBaseFloor = (tiles[i].Z == 7 && (tiles[i].ID & 0x7FFF) != 1);
					#endregion
		...
		public static void Designer_Roof( NetState state, IEntity e, EncodedReader pvSrc )
		{
			...
			if( context != null && (Core.SE || from.AccessLevel >= AccessLevel.GameMaster) )
			{
				...
				for( int i = 0; i < list.Length; i++ )
				{
					...
					#region SA
					if( mte.m_OffsetX == x && mte.m_OffsetY == y && GetZLevel( mte.m_OffsetZ, context.Foundation ) == context.Level  && (TileData.ItemTable[mte.m_ItemID & 0x7FFF].Flags & TileFlag.Roof) != 0 )
		...
		public static void Designer_RoofDelete( NetState state, IEntity e, EncodedReader pvSrc )
		{
			...
			if( context != null )	//No need to check if core.SE if trying to remvoe something that shouldn't be able to be placed anyways
			{
				...
				#region SA
				if( (TileData.ItemTable[itemID & 0x7FFF].Flags & TileFlag.Roof) == 0 )
				{
				#endregion
					design.SendDetailedInfoTo( state );
					return;
	...
	public class DesignStateDetailed : Packet
	{
			...
			: base( 0xD8 )
		{
			...
			for( int i = 0; i < tiles.Length; ++i )
			{
				...
				int z = mte.m_OffsetZ;
				#region SA
				bool floor = (TileData.ItemTable[mte.m_ItemID & 0x7FFF].Height <= 0);
				#endregion
				int plane, size;
