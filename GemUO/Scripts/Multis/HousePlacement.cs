			...
			for ( int x = 0; x < mcl.Width; ++x )
			{
				for ( int y = 0; y < mcl.Height; ++y )
				{
					...
					for ( int i = 0; i < addTiles.Length; ++i )
					{
						...
						#region SA
						TileFlag addTileFlags = TileData.ItemTable[addTile.ID & 0x7FFF].Flags;
						#endregion
						...
						for ( int j = 0; j < oldTiles.Length; ++j )
						{
							...
							#region SA
							ItemData id = TileData.ItemTable[oldTile.ID & 0x7FFF];
							#endregion
							...
							for ( int yOffset = -1; yOffset <= 1; ++yOffset )
							{
								...
								if ( vx >= 0 && vx < mcl.Width && vy >= 0 && vy < mcl.Height )
								{
									...
									for ( int i = 0; !shouldBreak && i < breakTiles.Length; ++i )
									{
										...
										#region SA
										if ( breakTile.Height == 0 && breakTile.Z <= 8 && TileData.ItemTable[breakTile.ID & 0x7FFF].Surface )
											shouldBreak = true;
										#endregion
									}
