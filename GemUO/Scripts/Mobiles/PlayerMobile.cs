...
namespace Server.Mobiles
{
    ...
    public class PlayerMobile : Mobile, IHonorTarget
    {
        ...
        public override void ToggleFlying()
        {
            ...
            {
             //edit by sid - added some landing stuff
                Freeze(TimeSpan.FromSeconds(1));
                Animate(61, 10, 1, true, false, 0);
                Flying = false;
                SendMessage("You have landed on the ground again.");
                //edit end
            ...
            else if (type != BlockMountType.None)
            {
                switch (type)
                {
                    ...
                    case BlockMountType.DismountRecovery:
                        {
                            SendLocalizedMessage(1112456); // You cannot fly while recovering from a dismount maneuver.
                            break;
                        }
                }
        ...
        private static void OnLogin(LoginEventArgs e)
        {
			...
			from.HairItemID = from.HairItemID;
			from.FacialHairItemID = from.FacialHairItemID;
			//from.Map = from.Map;
        }
